...
struct WrappedStats {
    ...
    static func compute(year: Int, trips: [Trip], allFeatures: [CountryFeature]) -> WrappedStats {
        ...
        for t in yearTrips {
            ...
            if !fs.isEmpty {
                for seg in fs {
                    ...
                    flightCount += max(1, outLegs + retLegs)  // al menos 1 si el seg existe
                ...
                //
                // La fórmula anterior `max(1, airports.count − 1)` cuenta 1
                // para cualquier round-trip directo (2 aeropuertos únicos),
                // de ahí el undercount reportado por el usuario.
        ...
        // Longest stay (días en un país concreto) — reparte correctamente los
        // días entre países cuando un mismo viaje cruza varios.
        //
        // Caso real del usuario: viaje a HKG 11 días que aterriza y despega de
        // HKG pero entre medias va a CHN y MAC (bus/foot/train). Los 11 días
        // NO son todos en HKG — se distribuyen según las transiciones entre
        // países que marcan los segments con sus dateFrom/dateTo.
        //
        // Algoritmo:
        //   1. Si un viaje tiene segments, caminamos cronológicamente. El país
        //      actual arranca en trip.isoCode; cada segment cambia el país
        //      actual al destino del tramo (último isoCode). El tiempo entre
        //      segments se acumula al país actual.
        //   2. Si no tiene segments, los días del trip van íntegros al país.
        //   3. Para grupos con children (multi-transport), usamos las
        //      estancias de los children directamente (ya representan per-país).
        //   4. Sumamos días por país y nos quedamos con el máximo.
        func daysBetween(_ a: Date, _ b: Date) -> Int {
            max(0, Calendar.current.dateComponents([.day], from: a, to: b).day ?? 0)
        }
        func daysInclusive(_ a: Date, _ b: Date) -> Int {
            max(1, daysBetween(a, b) + 1)
        }
        // Stays derivadas de los segments del primario.
        func staysFromPrimary(_ t: Trip) -> [String: Int] {
            let tripStart = t.dateFrom
            let tripEnd = t.dateTo ?? t.dateFrom
            let primaryIso = t.isoCode
            let segs = t.tripSegments.sorted { $0.dateFrom < $1.dateFrom }
            if segs.isEmpty {
                return [primaryIso: daysInclusive(tripStart, tripEnd)]
            }
            var perCountry: [String: Int] = [:]
            var currentIso = primaryIso
            var currentStart = tripStart
            for seg in segs {
                let segTime = seg.dateFrom
                // Acumula el tiempo transcurrido antes del tramo en el país actual.
                if segTime > currentStart {
                    perCountry[currentIso, default: 0] += daysBetween(currentStart, segTime)
                }
                // Transiciona al destino del tramo (último isoCode).
                let isos = seg.isoCodes.filter { !$0.isEmpty }
                if let dest = isos.last { currentIso = dest }
                currentStart = seg.dateTo ?? segTime
            }
            // Estancia final hasta el fin del trip.
            if tripEnd > currentStart {
                perCountry[currentIso, default: 0] += daysBetween(currentStart, tripEnd)
            }
            // Asegura que el país primario tenga al menos 1 día si el trip dura ≥1.
            if perCountry.isEmpty {
                perCountry[primaryIso] = daysInclusive(tripStart, tripEnd)
            }
            return perCountry
        }
        var perCountryDays: [String: Int] = [:]
        var groups: [String: [Trip]] = [:]
        var ungrouped: [Trip] = []
        for t in yearAllTrips where !t.isoCode.isEmpty {
            if let gid = t.segmentGroupID { groups[gid, default: []].append(t) }
            else { ungrouped.append(t) }
        }
        // Sin grupo: cada trip aporta sus stays derivadas de segments.
        for t in ungrouped {
            for (iso, days) in staysFromPrimary(t) {
                perCountryDays[iso, default: 0] += days
            }
        }
        // Con grupo: si hay children usamos sus fechas (ya representan per-país).
        // Si no hay children, usamos el primario con segments.
        for (_, members) in groups {
            let children = members.filter { $0.isSegmentChild }
            let primary = members.first(where: { !$0.isSegmentChild })
            if !children.isEmpty {
                for c in children {
                    perCountryDays[c.isoCode, default: 0] += daysInclusive(c.dateFrom, c.dateTo ?? c.dateFrom)
                }
                // Si el primario tiene días "sobrantes" (total − suma children) los
                // atribuimos al país del primario.
                if let primary {
                    let total = daysInclusive(primary.dateFrom, primary.dateTo ?? primary.dateFrom)
                    let used = children.reduce(0) { $0 + daysInclusive($1.dateFrom, $1.dateTo ?? $1.dateFrom) }
                    let leftover = max(0, total - used)
                    if leftover > 0 {
                        perCountryDays[primary.isoCode, default: 0] += leftover
                    }
                }
            } else if let primary {
                for (iso, days) in staysFromPrimary(primary) {
                    perCountryDays[iso, default: 0] += days
                }
            }
        }
                ...
                LazyVGrid(columns: cols, spacing: spacing) {
                    ForEach(Array(stats.allVisitedDestinations.enumerated()), id: \.offset) { pair in
                        if pair.element.flag == "🌐" {
                            ...
                            Text(pair.element.flag).font(.system(size: flagSize))
                        }
                    }
                }
                .padding(.horizontal, 24)

...
struct ContentView: View {
    ...
    private var multiContAchievedNow: Set<AchievementKind> {
        // Delega en `AchievementEvaluator` (pure struct, testable). Los
        // decodings (multiContinent, multiHemisphere, mapQuadrants) están
        // cacheados en @State refrescados via .onChange. La lógica completa
        // de 100+ logros vive ahora en AchievementEvaluator.swift donde sí
        // se puede testear con datos sintéticos.
        return AchievementEvaluator(
            trips: trips,
            countries: countries,
            countingMode: countingMode,
            multiContinentAssignments: cachedMultiContinentAssignments,
            multiHemisphereAssignments: cachedMultiHemisphereAssignments,
            mapQuadrants: cachedMapQuadrants,
            earnedPassportZones: earnedPassportZones,
            modernWondersRaw: modernWondersRaw
        ).evaluate()
    }
